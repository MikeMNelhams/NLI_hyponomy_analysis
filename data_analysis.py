...
class WordFrequencyHistogramFigureParams:
    def __init__(self, dataset_name="", metric="Word Frequency", x_label="Words", y_label="Frequency",
        ...
        self.file_save_path = "data/snli_1.0/word_frequency_figures/" + save_name
...
class DataAnalysis:
    ...
    def plot_word_histogram(self, sentence_name: str="sentence1", dataset: str="Training") -> None:
        term_count: dict = self.__data_loader.term_count(sentence_name)
        figure_params = WordFrequencyHistogramFigureParams(dataset_name=f"{dataset} data: {sentence_name}")
        ...
        values = frequencies[:figure_params.number_to_display, 1].astype(float)

        if figure_params.is_standardized:
            values = (values - np.mean(values)) / float(np.std(values))

        x_ticks = frequencies[: figure_params.number_to_display, 0]
...
def validation_word_frequency():
    ...
    validation_loader_analysis.plot_word_histogram("sentence1", dataset="validation")
    validation_loader_analysis.plot_word_histogram("sentence2", dataset="validation")
...
def test_word_frequency():
    ...
    test_loader_analysis.plot_word_histogram("sentence1", dataset="test")
    test_loader_analysis.plot_word_histogram("sentence2", dataset="test")
